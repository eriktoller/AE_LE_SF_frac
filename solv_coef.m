...
% Intersection
% zint = intersection_line(z1(1),z2(1),z1(2),z2(2));
...
%     if ii == N
%         theta(ii) = imag(log(chi_from_z(zint,z1(1),z2(1))));
%     end
%     if ii == N - 1
%         theta(ii) = imag(log(chi_from_z(zint,z1(2),z2(2))));
%     end
    for mm = 1:m
...
%         p(ii,jj) = -pressure( z(jj,ii),h,rho,g,@(z) Omega_w_func, @(Phi) fi_from_Phi( Phi,k ) );
%         pg(ii,jj) = imag( T_total(z(ii,jj),z1,z2,L,m,0.*beta,mu,sigma_11,g,rho,rho_w,k,@(z) 0*z,0,jj) );
...
%         for jj = 1:N
%             T_check(jj,ii) = T_total(z(ii,jj),z1,z2,L,m,beta,mu,sigma_11,g,rho,rho_w,k,Omega_w_func,0,ii);
%             T_check2(jj,ii) = T_total(z(ii,jj),z1,z2,L,m,beta,mu,sigma_11,g,rho,rho_w,k,Omega_w_func,ii,ii);
%         end
    end
    % Solver the tunnel
...
%     errora = max(max(abs(a_old-a)));
%     errorb = max(max(abs(b_old-b)));
%     errorbeta = max(max(abs(beta-beta_old)));
